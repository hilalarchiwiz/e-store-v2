...
const PAYMENT_OPTIONS = [
  ...
  },
  {
    value: "BANK_TRANSFER",
    label: "Bank Transfer",
    desc: "Transfer via bank to our account",
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const result = await placeOrder({
        addressId:
          !showNewForm && selectedAddressId ? selectedAddressId : undefined,
        addressData:
            ...
            {paymentMethod === "BANK_TRANSFER" && (
              <div className="mt-5 p-4 bg-amber-50 dark:bg-amber-900/10 rounded-2xl border border-amber-200 dark:border-amber-800/30">
                <p className="text-xs font-bold text-amber-700 dark:text-amber-400">
                  ...
                  details via email. Your order will be confirmed once payment
                  is verified.
                </p>
              </div>
            )}
