...
import { Star, StarIcon, StarOffIcon, StarsIcon } from "lucide-react";
...
const ProductItem = ({ item }) => {
  ...
  return (
    ...
    //         className="flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 text-blue-dark bg-white hover:bg-[#25a953]"
    //       >
    //         <svg
    ...
    //         className="inline-flex font-medium text-[14px] py-[7px] px-5 rounded-[5px] bg-blue text-white ease-out duration-200 hover:bg-blue-dark"
    //       >
    //         Add to cart
    //       </button>
    ...
    //           ${isInWishlist ? 'bg-red-500 text-white' : 'bg-white text-blue-dark hover:bg-[#25a953]'}`}
    //       >
    //         <svg
    //           className="fill-current"
    //           width="16"
    ...
    //     {
    //       item.discountedPrice ? <span className="text-dark-4 line-through">Rs. {item.price}</span> : ('')
    //     }
    //   </span>
    // </div>
    ...
    <Link href={`/shop-details/${item.id}`} className="block group border border-gray-200 rounded-lg p-4 transition-all duration-300 hover:border-blue hover:shadow-lg cursor-pointer">
      ...
      <span className="flex items-center gap-2 font-medium text-lg">
        <span className="text-dark">
          Rs.{" "}
          {discountPrice({ price: item.price, discount: item.discountedPrice })}
        </span>
