...
import { decreaseProductFromCart, increaseProductFromCart, removeProductFromCart } from "@/lib/action/home.action";
...
const SingleItem = ({ item, user }) => {
  ...
  const handleRemoveFromCart = async () => {
    const result = await removeProductFromCart(item.id, user?.id);
    if (result.success) {
  ...
  const handleIncreaseQuantity = async () => {
    const result = await increaseProductFromCart(item.id, user?.id)
    if (result.success) {
  ...
  const handleDecreaseQuantity = async () => {
    if (quantity > 1) {
      const result = await decreaseProductFromCart(item.id, user?.id)
      if (result.success) {
        setQuantity(quantity - 1);
        dispatch(updateCartItemQuantity({ id: item.id, quantity: quantity - 1 }));
        toast.success(result.message)
