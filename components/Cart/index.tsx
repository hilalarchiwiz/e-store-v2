...
import { getCarts, removeAllProductFromCart } from "@/lib/action/home.action";
import { addItemToCart, setCartItems } from "@/redux/features/cart-slice";
...
const Cart = () => {
  ...
  const fetchItems = async () => {
    try {
      // setLoading(true);
      const result = await getCarts(user?.id);
      if (result.success) {
        // dispatch(addItemToCart({
        // }));
        // toast.success(result?.message || "Failed to fetch");
      } else {
        toast.error(result?.message?.toString() || "Failed to fetch");
      }
    } catch (error: any) {
        ...
        <section className="overflow-hidden py-20 bg-gray-2">
          <div className=" max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
            <div className="flex flex-wrap items-center justify-between gap-5 mb-7.5">
              ...
              {/* <button onClick={() => emptyCart()} className="text-blue cursor-pointer">Clear Shopping Cart</button> */}
            </div>
