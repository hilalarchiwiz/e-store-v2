...
import { getProductToWishlists } from "@/lib/action/home.action";

const Header = ({ user, categories, setting, footer_setting }) => {
            ...
            <div className="hidden xl:flex items-center gap-3.5">
              ...
              <div>
                ...
                <p className="font-medium text-[15px] text-dark">
                  {footer_setting?.setting?.support_number}
                </p>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <nav>
                <ul className="flex xl:items-center flex-col xl:flex-row gap-5 xl:gap-6">
                  ...
                  <CategoryNavDropdown categories={categories} stickyMenu={stickyMenu} />
                </ul>
              </nav>
            </div>
            {/* // <!--=== Nav Right Start ===--> */}
            <div className="hidden xl:block">
              <ul className="flex items-center gap-5.5">
                <li className="py-4">
                  <Link href={'/recent-viewed'}
                    className="flex items-center gap-1.5 font-medium text-[15px] text-dark hover:text-blue"
                  >
                    <svg
                      className="fill-current"
                      width="16"
                ...
                <li className="py-4">
                  <Link
                    href="/wishlist"
                    className="group flex items-center gap-1.5 font-medium text-[15px] text-dark hover:text-blue relative"
                  >
                    <div className="relative">
                      <svg
                        className={`fill-current transition-colors duration-200 ${wishlistCount > 0 ? "text-blue" : "text-dark group-hover:text-blue"
                          }`}
                        width="16"
                        height="16"
                        viewBox="0 0 16 16"
                      ...
                      {/* 3. The Count Badge */}
                      {wishlistCount > 0 && (
                        <span className="absolute -top-2 -right-2 flex h-4 w-4 items-center justify-center rounded-full bg-blue text-[10px] text-white animate-in zoom-in duration-300">
                          {wishlistCount}
                        </span>
                      )}
                    </div>
