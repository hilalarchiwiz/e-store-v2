...
const Dropdown = ({ menuItem, stickyMenu }) => {
  ...
  return (
    ...
    >
      <a
        href="#"
        className={`hover:text-blue text-[14px] font-medium text-dark flex items-center gap-1.5 capitalize ${stickyMenu ? "xl:py-4" : "xl:py-6"
      ...
      >
        {menuItem.submenu.map((item, i) => (
          <li key={i}>
            <Link
              href={item.path}
              className={`flex text-[14px] hover:text-blue hover:bg-gray-1 py-[7px] px-4.5 ${pathUrl === item.path && "text-blue bg-gray-1"
                } `}
            >
              {item.title}
            </Link>
